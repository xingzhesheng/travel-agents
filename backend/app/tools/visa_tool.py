...
class VisaInput(BaseModel):
    destination: str = Field(description="目的地国家，如：日本、泰国、法国")


@tool("check_visa", args_schema=VisaInput)
def visa_tool(destination: str) -> str:
    """查询目的地签证要求，包括签证类型、状态、材料和办理时间。用户询问签证时调用。"""
    key = next((k for k in VISA_DB if k in destination or destination in k), None)
    if not key:
        return f"暂无 {destination} 的签证数据，建议访问目的地驻华大使馆官网查询最新要求，签证政策可能随时调整。"

    info = VISA_DB[key]
    materials = "\n".join(f"   {i + 1}. {m}" for i, m in enumerate(info["materials"]))

    return f"""📋 **{destination}签证信息**（中国大陆护照）
